# reference-api-py — app instructions
...
- Template: `api-py` (FastAPI, type: service)
- Role: **canonical reference** — copy these patterns into new Python services.
...
- Import building blocks from `getexp_core`; never re-implement them.
- Keep this app exemplary: it is what other services are modeled on.
- Keep the gates green (`uv run pytest && ruff check . && mypy ...`).
